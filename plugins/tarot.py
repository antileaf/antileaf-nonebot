...
major_tarot = [
    ...
    ('魔术师', 'The Magician', ('事情的开始，行动的改变，熟练的技术及技巧，贯彻我的意志，运用自然的力量来达到野心', '意志力薄弱，起头难，走入错误的方向，知识不足，被骗和失败')),
    ('女祭司', 'The High Priestess', ('开发出内在的神秘潜力，前途将有所变化的预言，深刻地思考，敏锐的洞察力，准确的直觉', '过于洁癖，无知，贪心，目光短浅，自尊心过高，偏差的判断，有勇无谋，自命不凡')),
    ...
    ('隐者', 'The Hermit', ('隐藏的事实，个别的行动，倾听他人的意见，享受孤独，自己的丢化，有益的警戒，年长者，避开危险，祖父，乡间生活', '无视警告，憎恨孤独，自卑，担心，幼稚思想，过于慎重导致失败，偏差，不宜旅行')),
    ('命运之轮', 'Wheel of Fortune', ('关键性的事件，有新的机会，因的潮流，环境的变化，幸运的开端，状况好转，问题解决，幸运之神降临', '挫折，计划泡汤，障碍，无法修正方向，往坏处发展，恶性循环，中断')),
    ...
    ('塔', 'The Tower', ('破产、逆境、被开除、急病、致命的打击、巨大的变动、受牵连、信念崩溃、玩火自焚、纷扰不断、突然分离，破灭的爱', '困境、内讧、紧迫的状态、状况不佳、趋于稳定、骄傲自大将付出代价、背水一战、分离的预感、爱情危机')),
...
def calc(x):
    ...
    return (x ^ y) + (pow(x, y, 1000000007) ^ pow(y, x, 998244353))
...
async def single_tarot(session):
    ...
    i = calc(user_id) % (2 * len(major_tarot))
